...
- **空态占位文案** `Ask me anything, or press / for shortcuts.`；终端窄于 62 列自动换成短句 `Ask me anything, or press /`。框内不再重复快捷键提示，输入框下面是**三行**：间距空行 + 常驻操作提示（只有一句 `连按两次 esc 退出`，左缘与框左竖线同列，武装时换成 `再按一次 esc 退出`）+ 反馈行（未知命令、主题切换等才出字）。空行是刻意的：提示紧贴框下沿太挤
